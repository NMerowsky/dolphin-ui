...
$laneBackup = "CASE
					WHEN (SELECT COUNT(*) FROM ngs_fastq_files WHERE aws_status = 2 AND ngs_lanes.id = ngs_fastq_files.lane_id) > 0 THEN '<td><button title=\"Currently uploading...\" class=\"btn btn-warning\"></td>'
					WHEN (SELECT COUNT(*) FROM ngs_fastq_files WHERE checksum != original_checksum AND (original_checksum != '' AND original_checksum IS NOT NULL) AND ngs_lanes.id = ngs_fastq_files.lane_id) > 0 THEN '<td><button title=\"The file checksums have changed from the original checksums\" class=\"btn btn-flickr\"></td>'
					WHEN (SELECT COUNT(*) FROM ngs_fastq_files WHERE checksum != backup_checksum AND (backup_checksum != '' AND backup_checksum IS NOT NULL) AND ngs_lanes.id = ngs_fastq_files.lane_id $amazon_str) > 0 THEN '<td><button title=\"Amazon checksum is mismatched\" class=\"btn btn-danger\"></td>'
					WHEN (SELECT COUNT(*) FROM ngs_fastq_files WHERE (backup_checksum = '' OR backup_checksum IS NULL) AND ngs_lanes.id = ngs_fastq_files.lane_id $amazon_str) > 0 THEN '<td><button title=\"Backup pending...\" class=\"btn\"></td>'
					WHEN (SELECT COUNT(*) FROM ngs_fastq_files WHERE date_modified < DATE_SUB(now(), INTERVAL 2 MONTH) AND ngs_lanes.id = ngs_fastq_files.lane_id $amazon_str) > 0 THEN '<td><button title=\"No errors, backup older than 2 months\" class=\"btn btn-primary\"></td>'
